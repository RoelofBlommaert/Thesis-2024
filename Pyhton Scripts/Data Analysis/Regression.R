...
nan_count <- sapply(model_data, function(x) sum(is.nan(x)))
print("NaN counts in each column:")
print(nan_count)
# Check for Inf values across the dataframe
inf_count <- sapply(model_data, function(x) sum(is.infinite(x)))
print("Inf counts in each column:")
print(inf_count)


#Make formulas for regression
lin_views_formula <- 'Views ~ Color.Complexity + Edge.Density + Luminance.Complexity + 
...
quad_likes_formula <- 'Likes ~ squared_Color_Complexity + squared_Edge_Density + squared_Luminance_Complexity + 
squared_Irregularity_of_OA + squared_Unique_Objects_Count + squared_Visual_Variety + logSubscribers + Time + Length + status'

lin_comment_formula <- 'Comments ~ Color.Complexity + Edge.Density + Luminance.Complexity +
Irregularity.of.Object.Arrangement + Unique.Objects.Count + Visual.Variety + logSubscribers + Time + Length + status'

quad_comment_formula <- 'Comments ~ squared_Color_Complexity + squared_Edge_Density + squared_Luminance_Complexity + 
squared_Irregularity_of_OA + squared_Unique_Objects_Count + squared_Visual_Variety + logSubscribers + Time + Length + status'
...
nb_model_likes <- glm.nb(lin_likes_formula, data = model_data, control = glm.control(maxit = 100))
...
control_settings <- glm.control(maxit = 100, epsilon = 1e-08, trace = TRUE)  # More conservative settings
nb_model_views_quad <- glm.nb(quad_views_formula, data = model_data, control = control_settings)
summary(nb_model_views_quad)
nb_model_likes_quad <- glm.nb(quad_likes_formula, data = model_data, control = control_settings)
