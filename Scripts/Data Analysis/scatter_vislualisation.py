...
comments = merged['commentCount']
likes = merged['likeCount']

# # Histograms
# plt.hist(views, bins=132)
...
# stats.probplot(views, dist="norm", plot=axes[0])
# axes[0].set_title('Views Q-Q Plot')

# stats.probplot(comments, dist="norm", plot=axes[1])
# axes[1].set_title('Comments Q-Q Plot')

# stats.probplot(likes, dist="norm", plot=axes[2])
# axes[2].set_title('Likes Q-Q Plot')

# plt.show()

# Histogram of the count data
sns.histplot(merged['viewCount'], kde=False)
plt.show()

# Pairplot to check for relationships and collinearity among variables
sns.pairplot(merged[['viewCount', 'likeCount', 'commentCount']])
plt.show()
